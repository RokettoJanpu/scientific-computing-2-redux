...
method = run( test, 1 );
p = k + run( test, 3 ); % order of the method
...
plot( g, '+' )
grid;
Mname = [num2str(cap(:,method)),'-',num2str(k)];
title(Mname);
fname = strcat(Mname,'.eps');
hold off
set(gca,'Fontsize',20)
%saveas(gcf,fname,'epsc') % if you want to save thefigures automatically
